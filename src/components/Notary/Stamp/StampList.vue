<template>
  <div v-if="is_notary" class="grid">
    <div
      v-for="(print, index) in prints.NotaryStamp"
      :key="index"
      class="border"
      :for="print.id"
    >
      <template v-if="print.user_id">
        <div class="position-relative">
          ...
          <a
            role="button"
            @click="deleteStamp(print.id)"
            class="text-danger btn-close d-block text-end delete"
          ></a>
        </div>
      </template>
  ...
  <div class="grid" v-if="!is_notary">
    <div
      v-for="(print, index) in prints.CompanyStamp"
      :key="index"
      class="border"
      :for="print.id"
    >
...
const { removePrint } = useActions({ removePrint: "print/removePrint" });
